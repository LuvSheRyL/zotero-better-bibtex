...
Zotero.BetterBibTeX.CSLDateParser = new function () {
    ...
    this.convertDateObjectToString = function(thedate) {
        ...
        for (var i = 0, ilen = 3; i < ilen; i += 1) {
            if (thedate[Zotero.CiteProc.CSL.DATE_PARTS_ALL[i]]) {
                ret.push(thedate[Zotero.CiteProc.CSL.DATE_PARTS_ALL[i]]);
            } else {
                break;
            }
        }
    ...
    this.parseDateToObject = function (txt) {
        ...
        //
        if (isRange) {
            for (var j=0,jlen=Zotero.CiteProc.CSL.DATE_PARTS_ALL.length; j<jlen; j++) {
                var item = Zotero.CiteProc.CSL.DATE_PARTS_ALL[j];
